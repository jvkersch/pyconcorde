...
state_capitals = pd.read_table(
    "us_state_capitals.tsv", header=None, names=["state", "city", "lat", "lon"]
)
...
# Instantiate solver
problem = Problem.from_coordinates(state_capitals.lat, state_capitals.lon, norm="GEO")

# Find tour
solution = run_concorde(problem)
# assert tour_data.success

solution = state_capitals.iloc[solution.tour]
print("Optimal tour:")
print(
    " -> ".join("{r.city}, {r.state}".format(r=row) for _, row in solution.iterrows())
)
...
ax.add_geometries([tour], ccrs.PlateCarree(), facecolor="none", edgecolor="red")
